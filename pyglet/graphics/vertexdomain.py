...
def _make_attribute_property(name: str) -> property:
    # The buffer is resolved through self.domain on every access, so vertex
    # lists remain correct across domain migration. get_region_for_write is
    # polymorphic: backed buffers mark dirty state for the next commit,
    # persistently mapped buffers wait on the domain's draw fence before
    # handing out a writable view of GPU-visible memory.
    def _attribute_getter(self: VertexList) -> Array[float | int]:
        return self.domain.attrib_name_buffers[name].get_region_for_write(self.start, self.count)

    def _attribute_setter(self: VertexList, data: Any) -> None:
        self.domain.attrib_name_buffers[name].set_region(self.start, self.count, data)
